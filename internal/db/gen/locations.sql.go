...
const searchOrgLocations = `-- name: SearchOrgLocations :many
WITH input AS (
  SELECT
    $1::uuid   AS org_id,
    $2::jsonb AS payload
...
type SearchOrgLocationsParams struct {
	OrgID   pgtype.UUID `db:"org_id" json:"org_id"`
	Payload []byte      `db:"payload" json:"payload"`
}

type SearchOrgLocationsRow struct {
...
func (q *Queries) SearchOrgLocations(ctx context.Context, arg SearchOrgLocationsParams) ([]SearchOrgLocationsRow, error) {
	rows, err := q.db.Query(ctx, searchOrgLocations, arg.OrgID, arg.Payload)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SearchOrgLocationsRow
	for rows.Next() {
		var i SearchOrgLocationsRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.CreatedAt,

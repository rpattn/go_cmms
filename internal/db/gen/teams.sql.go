...
const searchOrgTeams = `-- name: SearchOrgTeams :many
WITH input AS (
  SELECT
    $1::uuid   AS org_id,
    $2::jsonb AS payload
...
type SearchOrgTeamsParams struct {
	OrgID   pgtype.UUID `db:"org_id" json:"org_id"`
	Payload []byte      `db:"payload" json:"payload"`
}

type SearchOrgTeamsRow struct {
...
func (q *Queries) SearchOrgTeams(ctx context.Context, arg SearchOrgTeamsParams) ([]SearchOrgTeamsRow, error) {
	rows, err := q.db.Query(ctx, searchOrgTeams, arg.OrgID, arg.Payload)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SearchOrgTeamsRow
	for rows.Next() {
		var i SearchOrgTeamsRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.CreatedAt,
